...
if ($userInput -match '^(?!-)(?:[A-Za-z0-9-]{1,63}\.)+[A-Za-z]{2,}$') {
    ...
    if ($response.PSObject.Properties.Name -contains 'RequestResult') {
        $response.RequestResult.ResultDomainList
        ...
        $response.RequestResult.ResultDomainList | Out-File $outputFile
